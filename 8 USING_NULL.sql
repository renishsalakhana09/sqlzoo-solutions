...
*/
SELECT dept.name , COUNT(teacher.name) AS TOT_TCHR from teacher RIGHT JOIN dept ON (teacher.dept=dept.id) 
GROUP BY dept.name;
...
SELECT name , (CASE WHEN dept=1 or dept=2 THEN 'Sci' WHEN dept=3 THEN 'Art' ELSE 'None' 
END)
from teacher 
; 
